...
print "Variables post<br>";

print_r(array_keys($HTTP_POST_VARS));

print "<br><br>Variables get<br>";
...
/*
	...
	Carlos Andres Paz (capaz@colomsat.net.co) patched the parse_dynamic() and clear_dynamic() methods to

	allow HTML code on the same line than the dynamic block boundaries.
...
/*Cambios en archivo para php 7. Funciones ereg a preg_match, ereg_replace a preg_replace, split a preg_split y agregado de constructor para funcion FastTemplate*/
...
//	************************************************************
...

	{
		...
		if(getenv('WINDIR')!='') $this->WIN32 = true;
	...
	//El codigo de abajo de fasttemplate con la nomenclatura de clases de php4, no funciona en php7
...

	{
...

		{
...

			{
				...
				@debug("[FastTemplate] Warning: no value found for variable: $UnkVar ",0);
...

	{
...

		{
...

			{
				...
				$unknown = preg_split("\n",$template);
				//Linea de abajo comentada, porque a pesar de que each sirve en 7.0 a partir de 7.2 ya no
				...
				foreach($unknown as $key => $unknownval){
					$Element = $key;
					$Line = $unknownval;
					$UnkVar = $Line;
					if(!(empty($UnkVar)))
...
//	************************************************************
...

	{
...

		{
			...
			$openTag = "<!-- BEGIN DYNAMIC BLOCK: $Macro -->";

			$openTagSize = strlen($openTag);

			$closeTag = "<!-- END DYNAMIC BLOCK: $Macro -->";

			$closeTagSize = strlen($closeTag);



			$parsePos = 0;

			while ( $parsePos < $templateSize ) {

				$openTagPos = strpos($template, $openTag, $parsePos);

				if(is_int($openTagPos))

				{

					$newParent .= substr($template,$parsePos,$openTagPos-$parsePos);

					$macroStart = $openTagPos+$openTagSize;

					$closeTagPos = strpos($template, $closeTag, $macroStart);

					if(is_int($closeTagPos))

					{

						$newMacro .= substr($template,$macroStart,$closeTagPos-$macroStart);

						$newParent .= '{'."$MacroName}";

						$parsePos = $closeTagPos+$closeTagSize;
...

					{
						...
						$newParent .= substr($template,$macroStart);

						$parsePos = $templateSize;
...

				{
					...
					$newParent .= substr($template,$parsePos);
